...
export class PointsSchemeComponent implements OnInit {
	...
	constructor(private basketService: BasketService, public prizeService: PrizeService, public  messageServiceExt: MessageServiceExt, private confirmationService: ConfirmationService, private spinerService :SpinerService) {
	...
	editPoints(pointScheme: PointScheme) {
		this.prizeService.savePointScheme(pointScheme).subscribe(
			value => {
				this.refreshData();
				this.messageServiceExt.addMessageWithTime('success', 'Status', 'Dokonano edycji schematu pkt.', 5000);
			},
			error => {
	...
	addPoint(form: NgForm) {
		...
		if (this.pointSchemeToAdd.step && this.pointSchemeToAdd.points && this.pointSchemeToAdd.basketSezon) {
			...
			this.prizeService.savePointScheme(this.pointSchemeToAdd).subscribe(
				value => {
					this.messageServiceExt.addMessageWithTime('success', 'Status', 'Dodano schemat', 5000);
				}, error => {
					this.messageServiceExt.addMessageWithTime('error', 'Błąd', "Status: " + error._body + ' ', 5000);
