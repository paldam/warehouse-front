...
export class OrderAuditComponent implements OnInit {
	...
	constructor(orderService: OrderService, activatedRoute: ActivatedRoute) {
		this.revIdOfOrderStateAfterChange = +activatedRoute.snapshot.paramMap.get('revId');
		orderService.getOrderPrevStateFromHistoryByRevId(this.revIdOfOrderStateAfterChange).subscribe(
			data => {
				...
				this.customerDesc = data[0].customer.name + " | Firma: " + data[0].customer.company.companyName
					+ " | Telefon: " + data[0].customer.phoneNumber + " | Email: " + data[0].customer.email
					+ " | Dodatkowe: " + data[0].customer.additionalInformation;
			},
			null,
			() => {
		...
		orderService.getOrderStateFromHistoryByRevId(this.revIdOfOrderStateAfterChange).subscribe(
			data => {
				...
				this.customerAfterDesc = data[0].customer.name + " | Firma: " + data[0].customer.company.companyName
					+ " | Telefon: " + data[0].customer.phoneNumber + " | Email: " + data[0].customer.email
					+ " | Dodatkowe: " + data[0].customer.additionalInformation;
			},
			null,
	...
	getClassForCustomerDesc(): string {
		if (this.orderStateBeforeChange.customer.company.companyName === this.orderStateAfterChange.customer.company.companyName
			&& this.orderStateBeforeChange.customer.name === this.orderStateAfterChange.customer.name) {
			return 'redcolor3'
		} else {
			return 'redcolor'
		}
